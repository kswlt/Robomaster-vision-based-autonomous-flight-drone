...
### 6.4 调查在跨 bag 验证阶段被硬件中断（记录在案）

时间线（均为 2026-09-23）：

| 时刻 | 事件 |
|---|---|
| 12:16–12:17 | 板子发生**非正常复位**（wtmp 无 shutdown 记录），当时 SoC 47 °C |
| 12:17 之后 | D430 从 USB3(5000M) 掉到 USB2(480M)，软件无法恢复（§6.2） |
| 12:52 | 又一次非正常复位（串行批次启动约 35 秒后） |
| 12:58–13:10 | phase 2（ZUPT + 初始化 A/B）**完整跑完** ✅ |
| 13:10 起 | 跨 bag 验证启动，回放进行中 |
| **13:17 起** | **板子彻底失联**：ping 不通、SSH 超时、本网段 ARP 中已无其 MAC（`9c-b8-b4-5d-e7-26`），Tailscale 地址 `100.119.45.114` 亦不可达 |

对照同时刻本机网络正常（网关 `192.168.1.1` 4 ms 回应），因此**不是本侧网络问题**。

**因此以下工作未完成，且原因在硬件侧：**

* **跨 bag 验证**（用最优配置在 `LASEROFF` / `LASEROFF2` / `DYN50` / `DYN50B` 上重放，
  用于排除"最优 timeshift 只是拟合了单个 bag"）——**未完成**；
* **实时启动验证**（precheck 的 camera/imu 阶段、`verify_runtime.sh` 实测）——**未完成**；
* **实时性能测量**（IMU 实际速率在实时链路上的复现、visualization ON/OFF 对比）——**未完成**。

**恢复后应依次执行**（脚本均已就位）：

```bash
# 1) 前提：D430 必须回到 USB3（否则 precheck 会按设计拒绝启动）
lsusb -t | grep uvcvideo        # 必须看到 5000M

# 2) 实时链路验证
~/kswlt/vio_ws/start_vio.sh -d 42
~/kswlt/vio_replay/verify_runtime.sh 42 20

# 3) 跨 bag 验证（最优配置 = timeshift -0.015；建议叠加 zupt_chi2_multipler=1）
~/kswlt/vio_replay/cross_bag_check.sh ~/kswlt/vio_replay/configs/base_best
```

> 注：§5 的全部结论都来自**已固化在磁盘上的 immutable bag 与回放产物**，
> 不依赖相机在线，因此**不受本次中断影响**，仍然成立。
