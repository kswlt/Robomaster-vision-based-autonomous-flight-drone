...
| 7 | `scripts/replay_one.sh` / `analyze2.py` / `mksweep.py` / `sweep_*.sh` / `analyze_all.sh`（新增） | 可复现的离线回放与单变量 A/B 工具链；`mksweep.py` 保证除目标键外配置逐字节不变。 |
...
`zupt_m0`（当时破损值）/ `zupt_m1`（修正后）/ `zupt_m10`（极宽松）/ `zupt_off`（关闭）。
结果见 §5.5。

### 5.3 静态度初始化阈值

`init_d03`（0.3） vs `init_d10`（10.0）。结果见 §5.5。
