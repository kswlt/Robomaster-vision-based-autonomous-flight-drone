...
`tools/vision_mavlink_bridge.py` 将 `/orb_slam3/stereo/pose` 以 MAVLink `VISION_POSITION_ESTIMATE` 发送到同一条 PX4 USB 串口 `/dev/ttyACM0`，同时保留 `/imu/data` 发布。它不发送解锁、模式、起飞或执行器指令；默认检测到 PX4 armed 标志后会停止发送视觉位姿。只有明确使用 `--allow-armed` 启动参数才会在解锁后继续发送；该开关不应在坐标轴和低风险地面测试完成前启用。
