...
ROS2Visualizer::ROS2Visualizer(std::shared_ptr<rclcpp::Node> node, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  // Our tracking image
  it_pub_tracks = node->create_publisher<sensor_msgs::msg::Image>("trackhist", 2);
  PRINT_DEBUG("Publishing: %s\n", it_pub_tracks->get_topic_name());
  ...
  it_pub_loop_img_depth = node->create_publisher<sensor_msgs::msg::Image>("loop_depth", 2);
  it_pub_loop_img_depth_color = node->create_publisher<sensor_msgs::msg::Image>("loop_depth_colored", 2);
...
void ROS2Visualizer::publish_images() {
  ...
  if (false && it_pub_tracks->get_subscription_count() == 0)
    return;
...
void ROS2Visualizer::publish_loopclosure_information() {
  ...
  if (false && it_pub_loop_img_depth->get_subscription_count() != 0 || it_pub_loop_img_depth_color->get_subscription_count() != 0) {
